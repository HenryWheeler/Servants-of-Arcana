...
public class Ansi2 : ScreenSurface
{
    ...
    public Ansi2(IFont font, IFont.Sizes fontSize) : base(GameHost.Instance.ScreenCellsX, GameHost.Instance.ScreenCellsY)
    {
        ...
        // Create surface
        _ansiChild = new ScreenSurface(80, 55);
        ...
        // Read the ansi file
        using System.IO.Stream fileStream = new System.IO.FileStream("Resources/Title Screen.xp", System.IO.FileMode.Open);
        var image = REXPaintImage.Load(fileStream);

        // Convert all the image layers into cell surfaces
        var layers = image.ToLayersComponent();

        // Create a screen surface from the cell surface (just first layer for example)
        // SadConsole.ScreenSurface surface = new SadConsole.ScreenSurface(layers[0]);
    ...
    public Ansi2(IFont.Sizes fontSize) : this(GameHost.Instance.EmbeddedFont, fontSize) { }

    /// <summary>
    /// Creates the ansi splash screen using the <see cref="GameHost.EmbeddedFont"/> and a font size of <see cref="IFont.Sizes.One"/>.
    /// </summary>
    public Ansi2() : this(GameHost.Instance.EmbeddedFont, IFont.Sizes.One) { }
