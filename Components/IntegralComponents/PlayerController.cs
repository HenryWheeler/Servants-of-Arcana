    ...
    class KeyboardComponent : KeyboardConsoleComponent
    {
        ...
        public void Deny()
        {
            ...
            if (InteractionManager.popupActive)
            {
                ...
                else if (PrayerManager.isTerminalActive)
                {
                    InteractionManager.CreateAltarDisplay(PrayerManager.currentTerminal);
                }
            }
        ...
        public override void ProcessKeyboard(IScreenObject host, Keyboard info, out bool handled)
        {
            ...
            else if (Program.isGameActive)
            {
                if (Program.player.GetComponent<TurnComponent>().isTurnActive)
                {
                    ...
                    if (controller.autoPathing)
                    {
                        ...
                        else
                        {
                            ...
                            foreach (Vector vector1 in Program.dungeonGenerator.towerTiles)
                            {
                                if (Program.tiles[vector1.x, vector1.y] != null && Program.tiles[vector1.x, vector1.y].terrainType == 1 && !Program.tiles[vector1.x, vector1.y].GetComponent<Visibility>().explored)
                                {
                                    Vector vector = Program.tiles[vector1.x, vector1.y].GetComponent<Vector>();
                    ...
                    else
                    {
                        ...
                        else if (info.IsKeyDown(Keys.LeftShift) && info.IsKeyPressed(Keys.OemComma))
                        {
                            ...
                            {
                                if (Program.dungeonGenerator.staircaseUnlocked)
                                {
                                    confirming = true;
                                    Program.player.GetComponent<PlayerController>().confirming = true;
                                ...
                                {
                                    Log.Add("The path upward is locked.");
                                }
                            }
                ...
                {
                    if (info.IsKeyPressed(Keys.Escape)) { PrayerManager.CloseAltar(); }
                    else if (info.IsKeyPressed(Keys.D1)) { UseTerminal(true); }
                    else if (info.IsKeyPressed(Keys.D2)) { UseTerminal(false); }
                }
            }
        ...
        public void UseTerminal(bool first)
        {
            if (first)
            {
                confirming = true;
                Program.player.GetComponent<PlayerController>().confirming = true;
                keyboardEvent += FirstUse;

                InteractionManager.CreateConfirmationPrompt("Are you sure?");
            }
            else
            {
                confirming = true;
                Program.player.GetComponent<PlayerController>().confirming = true;
                keyboardEvent += SecondUse;
        ...
        {
            PrayerManager.UseAltar(true);
        }
        public void SecondUse() 
        {
            PrayerManager.UseAltar(false);
        }
