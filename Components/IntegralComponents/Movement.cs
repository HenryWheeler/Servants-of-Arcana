    ...
    public class Movement : Component
    {
        ...
        public void Move(Vector newPosition)
        {
            ...
            if (Math.CheckBounds(newPosition.x, newPosition.y))
            {
                ...
                {
                    if (newTraversable.GetComponent<Altar>() != null)
                    {
                        if (newTraversable.GetComponent<Altar>().active) 
                        {
                            PrayerManager.PrayAtAltar(newTraversable.GetComponent<Altar>());
                        }
                        else
                        {
                            Log.Add($"{Program.playerName} knows better than to request aid at the same altar more than once.");
                        }
                        return;
                    }
                    else
