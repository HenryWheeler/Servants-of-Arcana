...
namespace Servants_of_Arcana
{
    public class Look
    {
        ...
        public static void StartLooking()
        {
            ...
            Program.rootConsole.Children.MoveToBottom(Program.logConsole);
        ...
        public static void MoveReticle(Vector direction)
        {
            if (Math.CheckBounds(reticlePosition.x + direction.x, reticlePosition.y + direction.y))
            {
                ...
                if (!Program.tiles[reticlePosition.x, reticlePosition.y].GetComponent<Visibility>().visible)
                {
                    ...
                    Program.lookConsole.Print(4, Program.lookConsole.Height / 2 - 3, " You cannot see there.".Align(HorizontalAlignment.Center, Program.lookConsole.Width - 8, (char)177), Color.AntiqueWhite);
                }
