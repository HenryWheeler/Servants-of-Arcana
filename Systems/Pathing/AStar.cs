...
namespace Servants_of_Arcana
{
    public class AStar
    {
        ...
        public static List<Node> ReturnPath(Vector start, Vector end)
        {
            ...
            Node[,] currentNodeSet = new Node[Program.mapWidth, Program.mapHeight];
            ...
            while (openSet.Count > 0)
            {
                ...
                {
                    if (Program.tiles[neighbor.position.x, neighbor.position.y] == null || Program.tiles[neighbor.position.x, neighbor.position.y].terrainType == 0 || closedSet.Contains(neighbor))
                    {
                        continue;
                    }
        ...
        private static List<Node> ReturnNeighbors(Node[,] currentNodeSet, Node currentNode)
        {
            ...
            for (int x = currentNode.position.x - 1; x <= currentNode.position.x + 1; x++)
            {
                for (int y = currentNode.position.y - 1; y <= currentNode.position.y + 1; y++)
                {
                    ...
                    if (x >= 0 && y >= 0 && x < Program.mapWidth && y < Program.mapHeight)
                    {
        ...
        public AStar()
        {
            nodeSet = new Node[Program.mapWidth, Program.mapHeight];
